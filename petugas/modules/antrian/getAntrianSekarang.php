...
if (isset($_SERVER['HTTP_X_REQUESTED_WITH']) && ( $_SERVER['HTTP_X_REQUESTED_WITH'] == 'XMLHttpRequest' )) {
    ...
    $result = $mysqli->query("SELECT no_antrian FROM antrian WHERE tanggal='$hari_ini' AND status='1' ORDER BY no_antrian DESC LIMIT 1")
                              or die('Ada kesalahan pada query tampil nomor antrian: '.$mysqli->error);
    $rows = $result->num_rows;
    if ($rows <> 0) {
        $data = $result->fetch_assoc();
        $no_antrian = $data['no_antrian'];  
        echo "<h3 class='teal'>$no_antrian</h3>";
    } else {
